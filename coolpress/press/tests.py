...
class CategoryListCount(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.u = User.objects.create(first_name='juanito')
        cls.cu = CoolUser.objects.create(user=cls.u)
    ...
    def test_assert_list_size(self):


        cnt_of_categories = Category.objects.count()
        self.assertEqual(cnt_of_categories, 1)
    ...
    def test_assert_404(self):
        ...
        response = self.client.get('/post/2344')
        self.assertEqual(response.status_code, 404)
